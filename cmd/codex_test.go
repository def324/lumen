...
func TestCodexPathsFromEnv(t *testing.T) {
	...
	if paths.launcher != filepath.Join(pluginRoot, "scripts", "run.sh") {
		t.Fatalf("launcher = %q, want run.sh under plugin root", paths.launcher)
	}
...
func TestRunCodexInstall_WritesHookAndAddsMCPWhenMissing(t *testing.T) {
	...
	wantAdd := fmt.Sprintf("codex mcp add lumen -- %s stdio", filepath.Join(pluginRoot, "scripts", "run.sh"))
...
func TestRunCodexInstall_FailsWhenMCPAddFails(t *testing.T) {
	...
	t.Setenv("CODEX_HOME", filepath.Join(home, ".codex"))
	t.Setenv("LUMEN_PLUGIN_ROOT", pluginRoot)
	addCall := fmt.Sprintf("codex mcp add lumen -- %s stdio", filepath.Join(pluginRoot, "scripts", "run.sh"))
	runner := &fakeRunner{
...
func TestCodexMCPOutputMatchesRequiresCommandAndArgs(t *testing.T) {
	paths := codexPaths{launcher: "/repo/lumen/scripts/run.sh"}
	if !codexMCPOutputMatches([]byte("transport: stdio\ncommand: /repo/lumen/scripts/run.sh\nargs: stdio\n"), paths) {
		t.Fatal("expected command and args output to match")
	}
	if !codexMCPOutputMatches([]byte(`{"command":"/repo/lumen/scripts/run.sh","args":["stdio"]}`), paths) {
		t.Fatal("expected JSON command and args output to match")
	}
	if codexMCPOutputMatches([]byte("transport: stdio\nnote: /repo/lumen/scripts/run.sh\n"), paths) {
		t.Fatal("matched output without command field")
	}
	if codexMCPOutputMatches([]byte("command: /repo/lumen/scripts/run.sh\nnote: stdio\n"), paths) {
		t.Fatal("matched output without args field")
	}
	if codexMCPOutputMatches([]byte("command: /repo/lumen/scripts/run.sh.bak\nargs: stdio\n"), paths) {
...
func TestInstallCodexHookFile_PreservesHooksSymlink(t *testing.T) {
	...
	if _, err := installCodexHookFile(link, codexSessionStartCommand("/repo/lumen/scripts/run.sh")); err != nil {
		t.Fatalf("installCodexHookFile: %v", err)
	}
