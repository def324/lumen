...
func TestCodexLauncherPathForGOOS(t *testing.T) {
	...
	if got, want := codexLauncherPathForGOOS(pluginRoot, "darwin"), filepath.Join(pluginRoot, "scripts", "run.sh"); got != want {
...
func TestCodexSessionStartCommandQuotesLauncher(t *testing.T) {
	launcher := "/Users/franz/Code/lumen plugin/scripts/run.sh"
...
func TestCodexSessionStartCommandEscapesShellExpansion(t *testing.T) {
	launcher := "/Users/franz/Code/lumen $(touch nope)/scripts/run.sh"
...
func TestMergeCodexSessionStartHook_PreservesUnrelatedHooks(t *testing.T) {
	...
	raw := []byte(`{
		"hooks": {
			"PreToolUse": [{"matcher": "Bash", "hooks": [{"type": "command", "command": "\"/repo/lumen/scripts/run.sh\" hook session-start lumen --host claude"}]}],
	...
	doc := decodeCodexHooksDocument(t, out)
	if got := codexHookCommand(t, codexHookGroups(t, doc, "PreToolUse")[0]); got != `"/repo/lumen/scripts/run.sh" hook session-start lumen --host claude` {
	...
	raw := []byte(`{
		"hooks": {
			"SessionStart": [
				...
				{"matcher": "resume", "hooks": [{"type": "command", "command": "\"/repo/lumen/scripts/run.sh\" hook session-start lumen --host claude"}]}
			]
		}
	}`)
...
func TestMergeCodexSessionStartHook_PreservesForeignSessionStartCommands(t *testing.T) {
	command := codexSessionStartCommand("/repo/lumen/scripts/run.sh")
	foreign := `"/tmp/not-lumen/scripts/run.sh" hook session-start lumen --host claude`
			...
			"SessionStart": [
				{
					"matcher": "startup",
					"hooks": [
						{"type": "command", "command": "\"/repo/lumen/scripts/run.sh\" hook session-start lumen --host claude"},
...
func TestMergeCodexSessionStartHook_IdempotentOutsideLumenDirectory(t *testing.T) {
	command := codexSessionStartCommand("/repo/lumen fork/scripts/run.sh")
